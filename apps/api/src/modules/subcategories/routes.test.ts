...
test("subcategory route registration preserves public endpoint order", async () => {
  process.env.DATABASE_URL ??= "postgres://apex:apex@localhost:5432/apex_test";
  const { subcategoryRoutes } = await import("./routes");
  const { app, routes } = createRouteRecorder();

  await subcategoryRoutes(app as any, {} as any);
