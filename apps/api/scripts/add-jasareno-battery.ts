...
const ENTRIES = [
  { date: "2025-08-16", ref: "Q0988", amount: "8000.00" },
  { date: "2025-12-08", ref: "Q2247", amount: "1700.00" },
];

async function main() {
  // Create customer if not exists
  let [cust] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'JASARENO BATTERY'`));

  if (!cust) {
    const [maxPhone] = await db.execute(sql`SELECT MAX(SUBSTRING(phone FROM 4)::int) AS mx FROM customers WHERE org_id = ${ORG_ID} AND phone LIKE 'AR-%'`) as any[];
    const phone = `AR-${String((maxPhone?.mx ?? 100) + 1).padStart(4, "0")}`;
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "Jasareno Battery", phone, customerType: "SHOP",
      creditLimit: "0", paymentTermsDays: 30, currentBalance: "0", totalPurchases: "0", isActive: true,
    }).returning();
    cust = { id: created.id, name: created.name };
    console.log(`Created customer: ${cust.name} (${phone})`);
  }
  ...
  console.log(`${cust.name}: inserted ${inserted}, balance \u20B1${bal.toFixed(2)}, total \u20B1${tot.toFixed(2)}`);
  process.exit(0);
}
