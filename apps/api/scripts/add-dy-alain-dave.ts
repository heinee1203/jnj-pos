/**
 * Add 5 CHARGE transactions for Dy, Alain Dave.
 * Run: npx tsx apps/api/scripts/add-dy-alain-dave.ts
 */
...
const NEW_ENTRIES = [
  { date: "2025-12-08T00:00:00Z", ref: "Q2807", amount: "900.00" },
  { date: "2025-12-10T00:00:00Z", ref: "Q2824", amount: "5198.22" },
  { date: "2025-12-18T00:00:00Z", ref: "Q2934", amount: "4400.00" },
  { date: "2025-12-20T00:00:00Z", ref: "Q2948", amount: "38000.00" },
  { date: "2025-12-30T00:00:00Z", ref: "Q3036", amount: "551.05" },
];

async function main() {
  console.log("=== Add Dy, Alain Dave Transactions ===\n");

  let [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%dy%alain%")))
    .limit(1);

  if (!customer) {
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "Dy, Alain Dave", phone: "AR-DY-ALAIN", customerType: "INDIVIDUAL", currentBalance: "0", totalPurchases: "0", isActive: true,
  ...
  const existing = await db.execute(
    sql`SELECT reference_number FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number IN ('Q2807','Q2824','Q2934','Q2948','Q3036')`
  ) as any[];
