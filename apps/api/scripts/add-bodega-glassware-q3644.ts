/**
 * Add Q3644 CHARGE for Bodega Glassware.
 * Run: npx tsx apps/api/scripts/add-bodega-glassware-q3644.ts
 */
...
async function main() {
  console.log("=== Add Bodega Glassware Q3644 ===\n");

  const [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%bodega glass%")))
    .limit(1);
  ...
  const [existing] = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number = 'Q3644' LIMIT 1`) as any[];
  if (existing) { console.log("Q3644 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: customer.id, type: "CHARGE", amount: "23425.00",
    balanceAfter: "0", referenceType: "credit_sale", referenceNumber: "Q3644",
    notes: "Credit Sale", recordedAt: new Date("2026-03-04T00:00:00Z"),
  });
  console.log("  ADD CHARGE Q3644  2026-03-04  ₱23,425.00");
