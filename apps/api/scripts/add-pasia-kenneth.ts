/**
 * Add 3 CHARGE transactions for Pasia, Kenneth.
 * Run: npx tsx apps/api/scripts/add-pasia-kenneth.ts
 */
...
const NEW_ENTRIES = [
  { date: "2025-07-19T00:00:00Z", ref: "Q1097", amount: "12210.00" },
  { date: "2025-08-20T00:00:00Z", ref: "Q0366", amount: "4500.00" },
  { date: "2025-10-27T00:00:00Z", ref: "Q2025", amount: "2650.00" },
];

async function main() {
  console.log("=== Add Pasia, Kenneth Transactions ===\n");

  let [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%pasia%kenneth%")))
    .limit(1);

  if (!customer) {
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "Pasia, Kenneth", phone: "AR-PASIA", customerType: "INDIVIDUAL", currentBalance: "0", totalPurchases: "0", isActive: true,
  ...
  const existing = await db.execute(
    sql`SELECT reference_number FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number IN ('Q1097','Q0366','Q2025')`
  ) as any[];
