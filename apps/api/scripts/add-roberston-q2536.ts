/**
 * Add Q2536 CHARGE for Roberston Builders.
 * Run: npx tsx apps/api/scripts/add-roberston-q2536.ts
 */
...
async function main() {
  console.log("=== Add Roberston Builders Q2536 ===\n");

  let [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%roberston%")))
    .limit(1);

  if (!customer) {
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "Roberston Builders", phone: "N/A", customerType: "COMPANY", currentBalance: "0", totalPurchases: "0", isActive: true,
  ...
  const [existing] = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number = 'Q2536' LIMIT 1`) as any[];
  if (existing) { console.log("Q2536 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: customer.id, type: "CHARGE", amount: "6500.00",
    balanceAfter: "0", referenceType: "credit_sale", referenceNumber: "Q2536",
    notes: "Credit Sale", recordedAt: new Date("2025-12-05T00:00:00Z"),
  });
  console.log("  ADD CHARGE Q2536  2025-12-05  ₱6,500.00");
