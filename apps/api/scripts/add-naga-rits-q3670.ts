/**
 * Add Q3670 CHARGE for Naga Rits Corp.
 * Run: npx tsx apps/api/scripts/add-naga-rits-q3670.ts
 */
...
async function main() {
  console.log("=== Add Naga Rits Corp Q3670 ===\n");

  const [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%naga rits%")))
    .limit(1);
  ...
  // Idempotency
  const [existing] = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number = 'Q3670' LIMIT 1`) as any[];
  if (existing) { console.log("Q3670 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: customer.id, type: "CHARGE", amount: "2700.00",
    balanceAfter: "0", referenceType: "credit_sale", referenceNumber: "Q3670",
    notes: "Credit Sale", recordedAt: new Date("2026-03-04T00:00:00Z"),
  });
  console.log("  ADD CHARGE Q3670  2026-03-04  ₱2,700.00");
