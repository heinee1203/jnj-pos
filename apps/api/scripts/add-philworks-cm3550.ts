...
async function main() {
  const [cust] = await db.select({ id: customers.id }).from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'PHILWORKS CONSTRUCTION'`));
  if (!cust) { console.error("Not found"); process.exit(1); }

  const ex = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${cust.id} AND reference_number = 'CM-3550'`) as any[];
  if (ex.length > 0) { console.log("CM-3550 already exists."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: cust.id, type: "CREDIT_NOTE", amount: "650.00", balanceAfter: "0",
    referenceType: "credit_memo", referenceNumber: "CM-3550", notes: "Credit Memo", recordedAt: new Date("2025-10-15"),
  });
  ...
  console.log(`Philworks Construction: CM-3550 added. Balance \u20B1${bal.toFixed(2)}`);
  process.exit(0);
}
