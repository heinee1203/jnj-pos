...
const ENTRIES = [
  { date: "2025-12-22", ref: "Q3015", amount: "650.00" },
  { date: "2025-12-27", ref: "Q3030", amount: "215.00" },
];

async function main() {
  const [cust] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'NEW PRIMEMART'`));
