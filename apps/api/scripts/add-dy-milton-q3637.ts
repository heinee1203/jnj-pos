...
async function main() {
  const [cust] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'DY, MILTON'`));
  if (!cust) { console.error("Not found"); process.exit(1); }

  const ex = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${cust.id} AND reference_number = 'Q3637'`) as any[];
  if (ex.length > 0) { console.log("Q3637 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: cust.id, type: "CHARGE", amount: "9470.00", balanceAfter: "0",
    referenceType: "credit_sale", referenceNumber: "Q3637", notes: "Credit Sale", recordedAt: new Date("2026-03-04"),
  });
  ...
  console.log(`${cust.name}: Q3637 added. Balance \u20B1${bal.toFixed(2)}, Total \u20B1${tot.toFixed(2)}`);
  process.exit(0);
}
