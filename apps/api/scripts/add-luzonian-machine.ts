/**
 * Add 4 CHARGE transactions for Luzonian Machine Shop.
 * Run: npx tsx apps/api/scripts/add-luzonian-machine.ts
 */
...
const NEW_ENTRIES = [
  { date: "2025-10-15T00:00:00Z", ref: "Q2348", amount: "2800.00" },
  { date: "2025-10-17T00:00:00Z", ref: "Q2402", amount: "850.00" },
  { date: "2025-12-04T00:00:00Z", ref: "Q2526", amount: "600.00" },
  { date: "2025-12-12T00:00:00Z", ref: "Q2843", amount: "450.00" },
];

async function main() {
  console.log("=== Add Luzonian Machine Shop Transactions ===\n");

  let [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%luzonian%")))
    .limit(1);

  if (!customer) {
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "Luzonian Machine Shop", phone: "AR-LUZONIAN", customerType: "COMPANY", currentBalance: "0", totalPurchases: "0", isActive: true,
  ...
  const existing = await db.execute(
    sql`SELECT reference_number FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number IN ('Q2348','Q2402','Q2526','Q2843')`
  ) as any[];
