/**
 * Add Q3352 CHARGE for LGU - Ocampo.
 * Run: npx tsx apps/api/scripts/add-lgu-ocampo-q3352.ts
 */
...
async function main() {
  console.log("=== Add LGU - Ocampo Q3352 ===\n");
  ...
  const [existing] = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number = 'Q3352' LIMIT 1`) as any[];
  if (existing) { console.log("Q3352 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: customer.id, type: "CHARGE", amount: "45565.00",
    balanceAfter: "0", referenceType: "credit_sale", referenceNumber: "Q3352",
    notes: "Credit Sale", recordedAt: new Date("2026-01-26T00:00:00Z"),
  });
  console.log("  ADD CHARGE Q3352  2026-01-26  ₱45,565.00");
