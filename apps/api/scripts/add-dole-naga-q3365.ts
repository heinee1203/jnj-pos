/**
 * Add Q3365 CHARGE for DOLE - Naga City.
 * Run: npx tsx apps/api/scripts/add-dole-naga-q3365.ts
 */
...
async function main() {
  console.log("=== Add DOLE - Naga City Q3365 ===\n");

  let [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%dole%naga%")))
    .limit(1);

  if (!customer) {
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "DOLE - Naga City", phone: "AR-DOLE-NAGA", customerType: "GOVERNMENT", currentBalance: "0", totalPurchases: "0", isActive: true,
  ...
  const [existing] = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number = 'Q3365' LIMIT 1`) as any[];
  if (existing) { console.log("Q3365 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: customer.id, type: "CHARGE", amount: "10500.00",
    balanceAfter: "0", referenceType: "credit_sale", referenceNumber: "Q3365",
    notes: "Credit Sale", recordedAt: new Date("2026-02-02T00:00:00Z"),
  });
  console.log("  ADD CHARGE Q3365  2026-02-02  ₱10,500.00");
