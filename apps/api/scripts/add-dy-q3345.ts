...
async function main() {
  const [cust] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'DY, ALAIN DAVE'`));
  if (!cust) { console.error("Not found"); process.exit(1); }

  const existing = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${cust.id} AND reference_number = 'Q3345'`) as any[];
  if (existing.length > 0) { console.log("Q3345 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: cust.id, type: "CHARGE", amount: "550.00", balanceAfter: "0",
    referenceType: "credit_sale", referenceNumber: "Q3345", notes: "Credit Sale", recordedAt: new Date("2026-02-03"),
  });
  ...
  console.log(`Added Q3345 \u20B1550.00. Balance: \u20B1${bal.toFixed(2)}, Total: \u20B1${tot.toFixed(2)}`);
  process.exit(0);
}
