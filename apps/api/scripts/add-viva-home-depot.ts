/**
 * Add 2 CHARGE transactions for Viva Home Depot.
 * Run: npx tsx apps/api/scripts/add-viva-home-depot.ts
 */
...
const NEW_ENTRIES = [
  { date: "2025-12-22T00:00:00Z", ref: "Q2050", amount: "5600.00" },
  { date: "2025-12-29T00:00:00Z", ref: "Q2960", amount: "7285.00" },
];

async function main() {
  console.log("=== Add Viva Home Depot Transactions ===\n");

  const [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%viva home%")))
    .limit(1);
  ...
  const existing = await db.execute(
    sql`SELECT reference_number FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number IN ('Q2960', 'Q2050')`
  ) as any[];
  const existingRefs = new Set(existing.map((r: any) => r.reference_number));

  let inserted = 0;
  for (const entry of NEW_ENTRIES) {
    if (existingRefs.has(entry.ref)) { console.log(`  SKIP ${entry.ref} — already exists`); continue; }
