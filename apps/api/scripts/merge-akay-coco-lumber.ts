/**
 * Merge Akay Coco Lumber (typo) into Aky Coco Lumber.
 * Run: npx tsx apps/api/scripts/merge-akay-coco-lumber.ts
 */
...
async function main() {
  console.log("=== Merge Akay Coco Lumber → Aky Coco Lumber ===\n");

  const [source] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'AKAY COCO LUMBER'`));

  if (!source) { console.log("Akay Coco Lumber not found (already merged). Done."); process.exit(0); }

  const [target] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) = 'AKY COCO LUMBER'`));
  ...
  console.log(`${target.name}: balance=\u20B1${bal.toLocaleString("en-PH", { minimumFractionDigits: 2 })}, total=\u20B1${totalCharges.toLocaleString("en-PH", { minimumFractionDigits: 2 })}`);
