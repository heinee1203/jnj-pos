/**
 * Add Q0370 and Q0378 for Cabral, Eric Alfred.
 * Run: npx tsx apps/api/scripts/add-cabral-eric-alfred-2.ts
 */
...
const NEW_ENTRIES = [
  { date: "2025-10-23T00:00:00Z", ref: "Q0370", amount: "13500.00" },
  { date: "2025-11-25T00:00:00Z", ref: "Q0378", amount: "500.00" },
];

async function main() {
  console.log("=== Add Cabral, Eric Alfred Q0370 + Q0378 ===\n");

  const [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%cabral%eric alfred%")))
    .limit(1);
  ...
  const existing = await db.execute(
    sql`SELECT reference_number FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number IN ('Q0370','Q0378')`
  ) as any[];
