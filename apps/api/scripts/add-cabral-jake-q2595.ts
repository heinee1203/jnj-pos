...
async function main() {
  const [cust] = await db.select({ id: customers.id, name: customers.name })
    .from(customers).where(and(eq(customers.orgId, ORG_ID), sql`UPPER(${customers.name}) LIKE '%CABRAL%JAKE%'`));
  if (!cust) { console.error("Not found"); process.exit(1); }

  const ex = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${cust.id} AND reference_number = 'Q2595'`) as any[];
  if (ex.length > 0) { console.log("Q2595 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: cust.id, type: "CHARGE", amount: "13475.00", balanceAfter: "0",
    referenceType: "credit_sale", referenceNumber: "Q2595", notes: "Credit Sale", recordedAt: new Date("2025-12-09"),
  });
  ...
  console.log(`${cust.name}: Q2595 added. Balance \u20B1${bal.toFixed(2)}, Total \u20B1${tot.toFixed(2)}`);
  process.exit(0);
}
