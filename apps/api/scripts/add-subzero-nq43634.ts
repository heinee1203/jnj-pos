/**
 * Add NQ43634 CHARGE for Sub Zero - Naga City.
 * Run: npx tsx apps/api/scripts/add-subzero-nq43634.ts
 */
...
async function main() {
  console.log("=== Add Sub Zero - Naga City NQ43634 ===\n");

  let [customer] = await db
    .select({ id: customers.id, name: customers.name, balance: customers.currentBalance })
    .from(customers)
    .where(and(eq(customers.orgId, ORG_ID), ilike(customers.name, "%sub zero%")))
    .limit(1);

  if (!customer) {
    const [created] = await db.insert(customers).values({
      orgId: ORG_ID, name: "Sub Zero - Naga City", phone: "AR-SUBZERO", customerType: "COMPANY", currentBalance: "0", totalPurchases: "0", isActive: true,
  ...
  const [existing] = await db.execute(sql`SELECT id FROM customer_transactions WHERE customer_id = ${customer.id} AND reference_number = 'NQ43634' LIMIT 1`) as any[];
  if (existing) { console.log("NQ43634 already exists. Skipping."); process.exit(0); }

  await db.insert(customerTransactions).values({
    orgId: ORG_ID, customerId: customer.id, type: "CHARGE", amount: "85000.00",
    balanceAfter: "0", referenceType: "credit_sale", referenceNumber: "NQ43634",
    notes: "Credit Sale", recordedAt: new Date("2025-02-20T00:00:00Z"),
  });
  console.log("  ADD CHARGE NQ43634  2025-02-20  ₱85,000.00");
