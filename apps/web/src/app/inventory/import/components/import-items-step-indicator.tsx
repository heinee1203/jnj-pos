...
const STEPS = ["upload", "preview", "progress", "results"] as const;
const LABELS = ["Upload", "Preview", "Import", "Results"];

type ImportItemsStepIndicatorProps = {
  step: Step;
};

export function ImportItemsStepIndicator({ step }: ImportItemsStepIndicatorProps) {
  const stepIndex = STEPS.indexOf(step === "parsing" ? "upload" : step);
